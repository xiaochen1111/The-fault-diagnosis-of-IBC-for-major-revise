% Under the open loop control, the duty cycle varing will effect the fault 
% diagnosis result.
% The duty cycle is varying from 30% to 70%. 
% The simulation is Gate_signal_IBC_open_basis_test_PI_frequency_analysis_acc.
% For comments 1.1
%% Data Acquisition
% Load the Data
clc
...
%% Fault Diagnosis
% Change the fault detection rule
II1=[];
II2=[];
II3=[];
NN = length(Is1_Int);
...
if max([abs(round(((Is1_Int(ii)-Is2_Int(ii))),12)),abs(round(((Is2_Int(ii)-Is3_Int(ii))),12)),...
        abs(round(((Is1_Int(ii)-Is3_Int(ii))),12))])< (3e-05)                     %1.8000e-016e-05   
    IIs1_Int=1;
    IIs2_Int=2;
    IIs3_Int=3;
