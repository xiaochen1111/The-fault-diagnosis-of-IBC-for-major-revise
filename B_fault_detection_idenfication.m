...
% Load the Data
clc
N   = 322856:1:807426;%360001322870
...
T = linspace(t(4000),t(end),length(Max_difference));
pict = Plot(T,Max_difference,T,Y);
set(gca,'XLim',[0.008,0.02]);
% Picture Direct Result
color = '';
T = linspace(t(4000),t(end),length(II3));
figure(2)
subplot(211)
PPP = 0;
...
Picture_LCF;
set(gca,'XLim',[0.008,0.02]);
%% Plot for sbuSection A 1
% figure(10)
% subplot(211)
color = '';
PPP = 1;
plt = plot (t,I_sample1);
Picture_LCF;
hold on
plt = plot (t,I_sample2);
Picture_LCF;
hold on
plt = plot (t,I_sample3);
Picture_LCF;
set(gca,'YLim',[5,16])
legend('${I}_{in}(T1)$','${I}_{in}(T2)$','${I}_{in}(T3)$', 'Interpreter', 'latex');
ylabel('(a) I_{Sample}(A)','FontSize',15,'LineWidth',12)
set(gca,'XLim',[0.008,0.02])
%%
% subplot(212)
PPP = 1;
...
plt = plot (T,Is3_Int);
Picture_LCF;
set(gca,'YLim',[0.0014,0.0031])
set(gca,'XLim',[0.008,0.02])
ylabel('(b) I_{Integral}(A)','FontSize',15,'LineWidth',12) 
legend('$\hat{I}_{in}(T1)$','$\hat{I}_{in}(T2)$','$\hat{I}_{in}(T3)$', 'Interpreter', 'latex');
set(gcf,'position',[0 0 600 380])
%% Plot for sbuSection A 2
figure(9)
subplot(411)
color = 'r';
plt = plot (t,I_in);
...
set(gca,'YLim',[0.0014,0.0031])
ylabel('(b) I_{Integral}(A)','FontSize',15,'LineWidth',12) 
legend('$\hat{I}_{in}(T1)$','$\hat{I}_{in}(T2)$','$\hat{I}_{in}(T3)$', 'Interpreter', 'latex','FontSize',12);
subplot(413)
...
ylabel("(c) Threshold",'FontSize',15,'LineWidth',12);
subplot(414)
color = '';
PPP = 1;
plt=plot(T,II1);
...
Picture_LCF;
ylabel("(d) Indicator",'FontSize',15,'LineWidth',12);
legend("CI_{P1}","CI_{P2}","CI_{P3}",'FontSize',12);
set(gcf,'position',[0 0 600 500])

%% Plot for sbuSection B 2
subplot(311)
color = '';
PPP = 1;
plt = plot(t,iL1);
...
Picture_LCF;
legend('I_{L2}')
ylabel("Inductor current(A)",'FontSize',14.5)
subplot(313)
plt = plot(t,iL3);
Picture_LCF;
legend('I_{L3}')
