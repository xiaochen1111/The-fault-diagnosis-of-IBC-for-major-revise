...
{
    const real_T *ILi = (const real_T*) ssGetInputPortSignal(S,0);
    const real_T *Voi = (const real_T*) ssGetInputPortSignal(S,1);
    const real_T *Iref = (const real_T*) ssGetInputPortSignal(S,2);
    ...
    else if (P1.CNT ==0)
    {
       ...
       if (Voi[0]>100)
         {
        P1.CMP = iPI(vPI(V.REF,Voi[0]),IL[0]);
         }
    }
...
float iPI(float REF,float FB)
{
    ...
    return sat(I.I+I.P,0,P1.PRD*0.85);
}
